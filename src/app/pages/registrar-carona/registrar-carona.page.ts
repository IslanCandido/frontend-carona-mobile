    ...
    this.caronaService.verificarCarona(this.carona.rota.verificador, this.carona.usuario.cpf).subscribe(r => {
      if (r) {
        if (this.carona.id === null) {
          this.presentToast('Pedido de Carona já foi feito por esse usuário!');
        } else {
          if (this.carona.situacao == "Carona confirmada") {
            this.presentToast('Pedido de carona já foi confirmado, então não pode mais ser alterado!');
          } else {
            if (this.carona.usuario.cpf === this.carona.rota.veiculo.usuario.cpf) {
              this.presentToast('Não é possivel fazer um pedido de carona para a sua rota!');
            } else {
